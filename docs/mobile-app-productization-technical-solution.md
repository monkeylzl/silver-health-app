...
- 全局连通性监视器在启动、前后台切换、浏览器网络事件和每 15 秒定时检查时访问不缓存的 `GET /api/connectivity`；不单独依赖 iOS 不可靠的 `navigator.onLine`。
- 实际连通状态从离线恢复到在线时调用 `router.refresh()`，重新读取服务端数据。
- 离线状态卡可打开网络帮助，提供重新检测和系统设置路径；PWA 不调用 iOS 私有 URL Scheme，不能直接跳入系统网络设置。
- RSC 缓存只复用完整请求匹配的响应；不同 `_rsc` 或路由状态不得交叉复用，否则可能破坏 Next.js 客户端路由树。
- 不实现离线写入队列，避免重复健康记录。
